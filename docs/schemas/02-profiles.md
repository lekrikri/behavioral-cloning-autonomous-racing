...
> ⚠️ **État cible.** Le découpage `mask-worker + pd-policy` ci-dessous est la **cible**
> (décomposition perception/policy à venir). Aujourd'hui chaque profil mappe sur **un** worker
> auto monolithique (`controller_pd` / `inference_realcar`) — voir `configs/profiles.json`.
